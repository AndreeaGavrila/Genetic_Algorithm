...
def selectia(): 
    ...
    for _ in range(dimensiune_populatie):
        ...
        with open("evolutie2.txt", "a") as fout:
...
def incrucisare():
    ...
    np.random.shuffle(crm_incrucisare)    
...
def recombinare(h):
    ...
    for i in range(0, lung, 2): #se ia din 2 in 2 pt a avea mereu perechi
...
if __name__ == "__main__":
    ...
    for _ in range(etape):
        ...
        valoare_maxima2 = max(f)
        # calculez valorile maxime
        ...
        valoare_maxima3 = max(f)

        elitist_in = max(valoare_maxima1, valoare_maxima2, valoare_maxima3)
        elitist_fin = max(elitist_in, elitist_fin)

        # calculez valorile maxime
        val_maxim_func.append( elitist_fin )  

        # calculez valorile corespunzatoare pentru x
        for i in range(dimensiune_populatie):
            if f[i] == elitist_in:
                val_x.append( pop_baza_decimal[i] )  
    ...
    x_maxim = - 2.5275   # pentru x**3 + 3 * x**2 - 4 * x + 7  cu  -4 <= x <=  2
    # x_maxim = 3    # pentru x**3 + 3 * x**2 - 4 * x + 7  cu  -4 <= x <=  3
    print("\nX maxim: " + str(x_maxim) )

    x_funct = my_func(x_maxim)
    x_funct = 20.128  # pentru x**3 + 3 * x**2 - 4 * x + 7  cu  -4 <= x <=  2
    # x_funct = 49    # pentru x**3 + 3 * x**2 - 4 * x + 7  cu  -4 <= x <=  3
